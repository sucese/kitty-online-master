...
public class ModelManager {
    ...
    final static HashMap<Class<?>, BaseModel> mModelMap = new HashMap<>();
    ...
    public static void init(final Context ctx) {
        ...
        for (Class m : models) {
            if (m != null && BaseModel.class.isAssignableFrom(m)) {
                try {
                    BaseModel instance = (BaseModel) (m.newInstance());
                ...
                Log.e("Beam", "your model must extends BaseModel");
            }
        }
        //后台调用
        ...
        mBackThread.execute(new Runnable() {
            ...
            public void run() {
                for (Class m : finalModels) {
                    if (m != null)
                        mModelMap.get(m).onAppCreateOnBackground(ctx);
                }
            }
        });
